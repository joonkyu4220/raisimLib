...
PYBIND11_MODULE(RAISIMGYM_TORCH_ENV_NAME, m) {
  py::class_<VectorizedEnvironment<ENVIRONMENT>>(m, RSG_MAKE_STR(ENVIRONMENT_NAME))
    ...
    .def("getStateDim", &VectorizedEnvironment<ENVIRONMENT>::getStateDim)
    ...
    .def("setReferenceVelocity", &VectorizedEnvironment<ENVIRONMENT>::setReferenceVelocity)
    .def("getReference", &VectorizedEnvironment<ENVIRONMENT>::getReference)
