...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  void reset() final {
    speed = 0.8;//(double(rand() % 8) - 2.0) / 10.0;
    mode_ = 0;//rand() % 2;
  ...
  float step(const Eigen::Ref<EigenVec>& action) final {
    ...
    energy_reward_ = 0;
    action_reward_ = action.squaredNorm();
    // if (mode_ == 0)
    //   setReferenceMotion();
    // else
    //   setReferenceMotionBipedalMode();
    /// action scaling
    pTarget12_ = 1 * action.cast<double>() + gc_.tail(8);
  ...
  void computeReward() {
    ...
    for(auto& contact: solo8_->getContacts()) {
      if (phase_ <= max_phase_ / 8 * 2 && phase_ >= max_phase_ / 8 * 1) {
        if(solo8_->getBodyIdx("FL_LOWER_LEG") == contact.getlocalBodyIndex() || solo8_->getBodyIdx("FR_LOWER_LEG") == contact.getlocalBodyIndex()) {
          ...
          contact_reward += 2 * std::pow(vel1[0], 2); //0.01 for bounding
          contact_reward += 2 * std::pow(vel2[0], 2);
        }
      }
      else if (phase_ <= max_phase_ / 8 * 7 && phase_ >= max_phase_ / 8 * 6) {
        if(solo8_->getBodyIdx("HL_LOWER_LEG") == contact.getlocalBodyIndex() || solo8_->getBodyIdx("HR_LOWER_LEG") == contact.getlocalBodyIndex()) {
          ...
          contact_reward += 2 * std::pow(vel1[0], 2);
          contact_reward += 2 * std::pow(vel2[0], 2);
        }
      }
      else if (phase_ >= max_phase_ / 16 * 6 && phase_ <= max_phase_ / 16 * 10) {
        if(solo8_->getBodyIdx("HL_LOWER_LEG") == contact.getlocalBodyIndex() || solo8_->getBodyIdx("HR_LOWER_LEG") == contact.getlocalBodyIndex()
            || solo8_->getBodyIdx("FL_LOWER_LEG") == contact.getlocalBodyIndex() || solo8_->getBodyIdx("FR_LOWER_LEG") == contact.getlocalBodyIndex()) {
          contact_reward += 0.05 * std::pow(contact.getImpulse()[2], 2);
        }
      }
    }

    position_reward = 2.0 * std::pow(gv_[0]-speed, 2) + std::pow(gc_[1]-reference_[1], 2) + 0.0 * std::pow(gv_[2]-0.0, 2);
    contact_reward = std::exp(-1 * contact_reward);
    ...
    rewards_.record("joint", std::exp(-5*joint_reward)); //-3 for tracking controller
    rewards_.record("torque", energy_reward_);
    rewards_.record("contact", contact_reward);
    rewards_.record("action", action_reward_);
  }
  ...
  bool isTerminalState(float& terminalReward) final {
    ...
    quat[0] = gc_[3]; quat[1] = gc_[4]; quat[2] = gc_[5]; quat[3] = gc_[6];
    raisim::quatToRotMat(quat, rot);
    if (std::abs(gc_[4]) > 0.2 || std::abs(gc_[6]) > 0.2 || std::abs(gc_[5]) > 0.3 || gc_[2] < 0.2)// || std::abs(gc_[5] + 0.7071068) > 0.3)// || std::abs(gc_[4]) > 0.5 || std::abs(gc_[5]) > 0.5 || std::abs(gc_[6]) > 0.5 || rewards_.sum() < 0.5)
      return true;
    if (rewards_["contact"] < 0.45)
      return true;
